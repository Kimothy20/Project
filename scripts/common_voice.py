...
def process_file(row, clips_directory, directory, output_format):
    file_name = row['path']
    clips_name = file_name + '.' + output_format 
    text = clean_text(row['sentence'])
    audio_path = os.path.join(directory, 'clips', file_name + '.mp3')
    ...
    try:
        ...
        print(f"Skipping file due to SoxiError: {e}")
        return None
...
def main(args):
    ...
    if args.convert:
        ...
        for row in data_to_process:
            ...
            clips_name = file_name + '.' + args.output_format
    ...
    total = len(data)
    train_data = data[:int(total * (1 - percent / 100))]
    test_data = data[int(total * (1 - percent / 100)):]
